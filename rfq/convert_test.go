...
func TestConvertFindDecimalDisplayBoundaries(t *testing.T) {
	for _, btcPriceCents := range btcPricesCents {
		...
		for decDisp := 2; decDisp <= maxDecimalDisplay; decDisp++ {
			...
			maxUnits, smallestAmount, mSatPerUnit := calcLimits[BigInt](
...
func calcLimits[N Int[N]](btcPriceCent uint64, decDisplay int) (uint64, uint64, float64) {
	msatScale := 11
	...
	maxUnits := NewInt[N]().FromUint64(math.MaxUint64).Div(priceScaled.Value)
...
func TestConvertFixedPointFromUint64(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			scale: 8,
			expectedOut: FixedPoint[BigInt]{
				Value: newBig(uint64(10_000_000_000_000_000_000)),
				Scale: 8,
			},
		},
	}
...
func testMilliSatoshiToUnits[N Int[N]](t *rapid.T) {
	...
	unitsFloat := (float64(msat) / float64(btcutil.SatoshiPerBitcoin*1000)) * float64(unitsPerBtc) //nolint:lll
...
func testUnitsToMilliSatoshi[N Int[N]](t *rapid.T) {
	...
	//
	// TODO(roasbeef): make delta a function of scale
	msatFloat := (float64(units) / float64(unitsPerBtc)) * float64(btcutil.SatoshiPerBitcoin*1000) //nolint:lll
...
// TestConvertionMsat tests key invariant properties of the conversion
// functions.
func TestConversionMsat(t *testing.T) {
	...
	t.Run("roundtrip_conversion", rapid.MakeCheck(testRoundTripConversion[BigInt]))
}
