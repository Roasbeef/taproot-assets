...
func TestUniverseIssuanceProofs(t *testing.T) {
	...
	issuanceLeaves, err := multiverse.FetchMultiverseLeaves(
		ctx, nil, nil, universe.ProofTypeIssuance,
	)
	require.NoError(t, err)
	require.Len(t, issuanceLeaves, 0)
	transferLeaves, err := multiverse.FetchMultiverseLeaves(
	...
	multiverseLeaves, err := multiverse.FetchMultiverseLeaves(
...
func TestMultiverseRootSum(t *testing.T) {
	...
	runTestCase := func(t *testing.T, tc testCase) {
		...
		// We now delete the whole universes and expect the multiverse
		// leaves to also disappear.
