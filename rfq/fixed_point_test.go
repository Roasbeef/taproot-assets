...
func testScaleTo[N Int[N]](t *rapid.T) {
	...
	scale := rapid.IntRange(0, 18).Draw(t, "scale")
	newScale := rapid.IntRange(0, 18).Draw(t, "newScale")

	fp := FixedPointFromUint64[N](value, scale)
	scaledFp := fp.ScaleTo(newScale)

	// Scaling to the same scale should not change the value.
	if scale == newScale {
		require.True(
			t, fp.Equals(scaledFp), "scaling to same scale "+
				"changed value: %v to %v", fp, scaledFp,
		)
	}

	// Scaling up then down should approximate the original value.
	if newScale > scale {
...
func testFixedPointMultiplication[N Int[N]](t *rapid.T) {
	...
	scale := rapid.IntRange(0, 9).Draw(t, "scale")
...
func testFixedPointDivision[N Int[N]](t *rapid.T) {
	...
	scale := rapid.IntRange(1, 9).Draw(t, "scale")
...
func testEquality[N Int[N]](t *rapid.T) {
	...
	fp1 := FixedPointFromUint64[N](value, scale)
	fp2 := FixedPointFromUint64[N](value, scale)

	// Two FixedPoints with the same value and scale should be equal.
	require.True(
	...
	fp3 := FixedPointFromUint64[N](value+1, scale)
	require.False(
	...
	fp4 := FixedPointFromUint64[N](value, scale+1)
	require.False(
...
func testFromUint64[N Int[N]](t *rapid.T) {
	...
	// Scaling back to 0 should give the original value.
	scaledBack := fp.ScaleTo(0)
	require.Equal(t, value, scaledBack.Value.ToUint64())
}
